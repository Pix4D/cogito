...
- `gchat_webhook`. Default: empty. URL of a [Google Chat webhook]. A notification about the build status will be sent to the associated chat channel, using the pipeline name as thread ID. All build statuses expect "pending" will trigger a notification. See section [Effects on Google Chat](#effects-on-google-chat) (since v0.7.0).
