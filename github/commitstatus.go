...
func (cs CommitStatus) Add(sha, state, targetURL, description string) error {
	...
	if err := cs.target.Retry.Do(Backoff, Classifier,
		func() error {
			...
			return NewGitHubError(httpResp, errors.New(strings.TrimSpace(string(body))))
		}); err != nil {
...
// FIXME can I embed this in ghErr.Error() instead??? Seems to make sense...
